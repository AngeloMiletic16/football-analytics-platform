...
MODEL_OUTPUT_PATH = PROJECT_ROOT / "artifacts" / "models" / "ft_result_lr_v1.joblib"
METADATA_OUTPUT_PATH = PROJECT_ROOT / "artifacts" / "metadata" / "ft_result_lr_v1.json"

TARGET_COLUMN = "ft_result"
...
def prepare_training_frame(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.Series]:
    ...
    y = model_df[TARGET_COLUMN].astype(str).copy()
...
def evaluate_model(model: Pipeline, X_test: pd.DataFrame, y_test: pd.Series) -> dict:
    ...
    metrics = {
        ...
        "log_loss": round(float(log_loss(y_test, y_proba, labels=class_labels)), 6),
    }
    return metrics
...
def main() -> None:
    ...
    class_labels = [str(x) for x in model.named_steps["model"].classes_]
