...
MODEL_OUTPUT_PATH = PROJECT_ROOT / "artifacts" / "models" / "over_2_5_lr_v1.joblib"
METADATA_OUTPUT_PATH = PROJECT_ROOT / "artifacts" / "metadata" / "over_2_5_lr_v1.json"
...
def evaluate_model(model: Pipeline, X_test: pd.DataFrame, y_test: pd.Series) -> dict:
    ...
    metrics = {
        ...
        "log_loss": round(float(log_loss(y_test, y_proba)), 6),
    }
    return metrics
...
def main() -> None:
    ...
    print(f"Target positive rate ({TARGET_COLUMN}=1): {y.mean():.4f}")
